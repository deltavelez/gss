...
void lb_co_color(S_INT_16_T color)
{
  printf("\x1b[%dm",color);
}
...
void lb_co_color_bg_216(S_INT_16_T r, S_INT_16_T g, S_INT_16_T b)
{
  if ((r>5) || (g>5) || (b>5))
    {
...
void lb_co_color_fg_216(S_INT_16_T r, S_INT_16_T g, S_INT_16_T b)
{
  if ((r>5) || (g>5) || (b>5))
    {
      printf("Error: lb_co_textcolorRGB --> value higher than 5\r\n");
...
void lb_co_cursor_shift(S_INT_16_T x, S_INT_16_T y)
{
...
void lb_co_set_echo(S_INT_8_T state) 
{ 
...
void lb_co_gotox(S_INT_16_T x)
{
  printf("\x1b[%dG",x);
}

void lb_co_gotoxy(S_INT_16_T x, S_INT_16_T y)
{
  printf("\x1b[%d;%df",y,x);
  //  printf("\x1b[%d;%dH",x,y);  /* Equivalent */
}

S_INT_8_T lb_co_kbhit()
{
...
void lb_co_printf_block(char *str, S_INT_16_T a, S_INT_16_T b)
{
  S_INT_16_T i;
  if ( (a<=b) && (b<strlen(str)) )
    {
...
void lb_co_printf_firstn(char *str, S_INT_16_T n)
{
  S_INT_16_T i=0;
  while ( (i<n) && (str[i]!='\0') )
    {
...
void lb_co_printf_lastn(char *str, S_INT_16_T n)
{
  S_INT_16_T i;
  if (n<strlen(str))
    {
