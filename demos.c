...
#define MAX_N 10
  ...
  /* a external tool such as ffmpeg. Examples are available at:                 */
  /* https://trac.ffmpeg.org/wiki/Slideshow                                     */
  /* ffmpeg -framerate 12 -i img%03d.png output.mp4
  /******************************************************************************/
...
#define DEMO_VIDEO
  ...
  COMPLEX_T z, p;
  VIEWPORT_2D_T win;
  lb_gr_SDL_init("Hola", SDL_INIT_VIDEO, 320, 200, 0, 0, 0);
  ...
  /* 12 frames per second, 3 minutes long */
  for(k=0;k<3*60*12;k++)
    {
	...
	for(yp=0;yp<win.yp_max;yp++)
      ...
      lb_gr_refresh();
      z_zoom*=1.001;
      sprintf(filename,"./media/videos/man%06ld.jpg",k);
      lb_gr_JPGfile_save(filename, NULL, 85);
    }
  lb_gr_SDL_close();
  SDL_Quit();
  return EXIT_SUCCESS;
